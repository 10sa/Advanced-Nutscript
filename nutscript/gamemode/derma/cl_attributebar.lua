...
function PANEL:Init()
	...
	self.content.Paint = function(panel, w, h)
		...
		surface.DrawTexturedRect(1 + border, 1 + border, math.Max(1, width - (2 + border*2)), h - (2 + border*2))
		
		surface.SetDrawColor(0, 0, 0, 255);
		surface.DrawOutlinedRect(1 + border, 1 + border, math.Max(1, width - (2 + border*2)), h - (2 + border*2));
	end
...
function PANEL:SetTooltip(toolTip)
	self.content:SetTooltip(toolTip);
end;
