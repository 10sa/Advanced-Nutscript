...
function META:NewVar(name, value, state, noSave)
	...
	if (dataType == "table") then
		self.dataTypes[name] = pon.decode
...
if (SERVER) then
	...
	function nut.char.LoadID(client, index, callback)
		...
		local tables = "money, chardata, charname, inv, description, faction, model, gender";
	...
	function nut.char.SendInfo(client, index, callback)
		...
		local steamID = client:SteamID64() or 0
		local condition = "steamid = "..steamID.." AND id = "..index..sameSchema()
		local tables = "charname, faction, id, description, model, chardata, gender"
		...
		nut.db.FetchTable(condition, tables, function(data)
			if (IsValid(client)) then
				if (data and table.Count(data) > 0) then
					...
					netstream.Start(client, "nut_CharInfo", {data.charname, data.description, data.model, data.faction, data.id, data.skin, data.gender, banned})
	...
	function nut.char.Save(client)
		...
		local bodygroups = {};
		for index, bodygroup in pairs(client:GetBodyGroups()) do
			table.insert(bodygroups, {bodygroup.id, client:GetBodygroup(bodygroup.id)});
		end;
		...
		character:SetData("bodygroup", bodygroups);
	...
	netstream.Hook("nut_CharInfo", function(data)
		...
		local gender = data[7];
		local banned = util.tobool(data[8])
