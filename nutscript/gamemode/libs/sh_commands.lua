...
if (SERVER) then
	...
	function nut.command.RunCommand(client, action, arguments, noMsgOnFail, starArgs)
		...
		elseif (!noMsgOnFail) then
			nut.util.Notify(nut.lang.Get("nocommend"), client)
		end
	...
	hook.Add("BuildHelpOptions", "nut_CommandHelp", function(data, tree)
		...
		data:AddHelp("명령어", function(tree)
			local html = ""
			...
			for k, v in SortedPairs(nut.command.buffer) do
				if (!v.category) then
					...
					local syntax = v.syntax or "[인자 없음]"
					...
					html = html.."<p><b>/"..k.."</b><br /><hi><i>인자 :</i> "..syntax.."</p>"
				end
			end
		...
		data:AddCallback("명령어", function(node, body)
			for k, v in SortedPairs(nut.command.buffer) do
				if (v.category) then
					if (v.adminOnly and !LocalPlayer():IsAdmin()) then
						continue
