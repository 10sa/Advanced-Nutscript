...
BASE.category = nut.lang.Get("weapon")
...
BASE.functions.Equip = {
	text = nut.lang.Get("wp_equip"),
	run = function(itemTable, client, data)
		if (SERVER) then
			if (client:HasWeapon(itemTable.class)) then
				nut.util.Notify(nut.lang.Get("already_equip_weapon"), client)

				return false
			end

			if (nut.config.noMultipleWepSlots and IsValid(client:GetNutVar(itemTable.type))) then
				nut.util.Notify(nut.lang.Get("already_wear_parttype", itemTable.type), client)
			...
			AdvNut.hook.Run("OnWeaponEquipped", client, itemTable, true)
		end
	end,
...
BASE.functions.Unequip = {
	...
	run = function(itemTable, client, data)
		if (SERVER) then
			...
			client:UpdateInv(itemTable.uniqueID, 1, newData, true)
			AdvNut.hook.Run("OnWeaponEquipped", client, itemTable, false)
			return true
		end
	end,
	shouldDisplay = function(itemTable, data, entity)
...
if (SERVER) then
	AdvNut.hook.Add("PlayerSpawn", "nut_WeaponBase", function(client)
