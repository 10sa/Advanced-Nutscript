function GM:ShowHelp(client) end
...
function GM:OnLocalPlayerValid(client, steamID, uniqueID)
	netstream.Start(client, "nut_LoadingData", nut.lang.Get("sendTime"))
	netstream.Start(client, "nut_CurTime", nut.util.GetTime())

	client:KillSilent()
	client:StripWeapons()

	netstream.Start(client, "nut_LoadingData", nut.lang.Get("initchardata"))
	client:InitializeData()

	player_manager.SetPlayerClass(client, "player_nut")
	player_manager.RunClass(client, "Spawn")

	netstream.Start(client, "nut_LoadingData", nut.lang.Get("loadchar"))
	nut.char.Load(client, function()
		netstream.Start(client, "nut_LoadingData", nut.lang.Get("otherloadchar"))
			...
			if (v.character and v != client) then
				...
				timer.Simple(k * fraction, function()
					if (IsValid(client) and IsValid(v)) then
						...
						netstream.Start(client, "nut_LoadingData", nut.lang.Get("charnetworking")..math.Round((i / total) * 100).."%")
...
function GM:PlayerLoadedChar(client)
	...
	if (!client:GetNutVar("sawCredits")) then
		...
		timer.Simple(15, function()
			...
			nut.scroll.Send("Advanced Nutscript : "..nut.lang.Get("schema_author", "Tensa").." / "..nut.lang.Get("based_framework"), client, function()
				if (IsValid(client)) then
					nut.scroll.Send(SCHEMA.name..": "..nut.lang.Get("schema_author", SCHEMA.author), client, function()
					end)
				end
			end)
		end)
...
function GM:PlayerSpawn(client)
	...
	if (client.character:GetData("bodygroup")) then
		for index, data in pairs(client.character:GetData("bodygroup")) do
			client:SetBodygroup(data[1], data[2]);
		end;
	end;
...
function GM:GetGameDescription()
	return "AdvNS - "..(SCHEMA and SCHEMA.name or "Unknown")
end
...
function GM:ShutDown()
	MsgN("Advanced Nutscript is shutting down...")
...
function GM:PlayerDeath(victim, weapon, attacker)
	...
	timer.Simple(0, function()
		victim:SetMainBar(nut.lang.Get("spawning"), nut.config.deathTime)
	...
	if (key == IN_USE) then
		...
		if (IsValid(entity) and client:GetNetVar("tied")) then
			if (client:GetNutVar("nextTieMsg", 0) < CurTime() and SERVER) then
				nut.util.Notify(nut.lang.Get("tied"), client)
				client:SetNutVar("nextTieMsg", CurTime() + 1)
			end
...
function GM:PlayerBoughtItem(client, itemTabletr) end
