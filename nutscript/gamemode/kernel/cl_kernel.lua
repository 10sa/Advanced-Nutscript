...
function GM:PaintBar(value, color, x, y, width, height)
	...
	if (width > 0) then
		...
		surface.SetMaterial(Material("nutscript/vignette.png"));
...
local BAR_WIDTH, BAR_HEIGHT = ScrW() * NUT_CVAR_BWIDTH:GetFloat(), NUT_CVAR_BHEIGHT:GetInt() * 1.1
local lastFPS = 0
local avgFPS = 0
...
function GM:DrawPlayerDamageScreen(damageValue)
	...
	surface.SetDrawColor(255, 255, 255, math.Clamp(255 * damageValue, 0, 255));
	surface.SetMaterial(Material("advnutscript/damage_overlay.png"));
...
function GM:DrawVignette()
	if (AdvNut.hook.Run("IsCanDrawingVignette")) then
		...
		surface.SetDrawColor(Color(5, 5, 5, 80));
		surface.DrawRect(0, 0, ScrW(), ScrH());
			
		surface.SetDrawColor(Color(50, 0, 0, 10));
		surface.DrawRect(0, 0, ScrW(), ScrH());
		
		surface.SetDrawColor(Color(0, 50, 0, 10));
		surface.DrawRect(0, 0, ScrW(), ScrH());
		
		surface.SetDrawColor(Color(0, 0, 50, 10));
		surface.DrawRect(0, 0, ScrW(), ScrH());
	end;
end;
...
function GM:HUDPaintTargetPlayer(client, x, y, alpha)
	...
	if (!client:GetNutVar("descLines") or description != (client:GetNutVar("descText") or "")) then
		...
		local descLines, _, lineH = nut.util.WrapText("AdvNut_EntityDesc", ScrW() * 0.4, client:GetNutVar("descText"))

		client:SetNutVar("descLines", descLines)
		client:SetNutVar("lineH", lineH)
	end

	y = nut.util.DrawWrappedText(x, y, client:GetNutVar("descLines"), client:GetNutVar("lineH"), "AdvNut_EntityTitle", 1, 1, alpha)

	if (client:GetNetVar("tied")) then
		local add = math.sin(RealTime() * 3)*25 + 50

		nut.util.DrawText(x, y, "Press <Use> to untie.", Color(math.min(255 + add, 255), math.min(165 + add, 255), math.min(30 + add, 255)), "AdvNut_EntityDesc")
	end
end
...
function GM:GetUnknownPlayerName(client)
	return nut.lang.Get("unknown_player")
end

function GM:IsCanDrawingVignette() 
	return true;
end;

function GM:IsCanDrawingBackgroundBlur() 
	return true;
end;
