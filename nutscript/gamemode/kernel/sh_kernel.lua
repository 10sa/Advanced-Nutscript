...
function GM:OnReloaded()
	if (SERVER and !isCalled) then
		...
		nut.util.AddLog("Advanced Nutscript has been auto-refreshed.", LOG_FILTER_MAJOR)
	end
...
function GM:SetupAttributes()
	if (nut.config.baseAttributes) then
		ATTRIB_STR = nut.attribs.SetUp(nut.lang.Get("strength"), nut.lang.Get("strength_desc"), "str");
		
		ATTRIB_ACR = nut.attribs.SetUp(nut.lang.Get("acrobatics"), nut.lang.Get("acrobatics_desc"), "acr", function(client, points)
...
function GM:CalcMainActivity(client, velocity)
	...
	if (client.character and client:Alive()) then
		...
		elseif (client:InVehicle()) then
			...
			local vehicleTb = animClass.vehicle
			local class = "chair"
			
			if (vehicleTb and vehicleTb[class]) then
			
			local act = vehicleTb[class][1]
			local fixvector = vehicleTb[class][2]
			
			if (fixvector) then
				client:ManipulateBonePosition(0, fixvector)
			end
			
			if ( act ) then
				if ( type( act ) == "string" ) then
					client.CalcSeqOverride = client:LookupSequence(vehicleTb[class][1])
				else
					client.CalcIdeal = act
				end
			end
		end
