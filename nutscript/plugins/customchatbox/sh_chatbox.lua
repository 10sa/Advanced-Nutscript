if (CLIENT) then
			...
			for k, v in ipairs({...}) do
				...
				elseif (type(v) == "IMaterial" or type(v) == "table" and type(v[1]) == "IMaterial") then
					...
					data = data..v
						...
						data = data:gsub("%b**", function(value)
						local inner = value:sub(2, -2)

						if (inner:find("%S")) then
							return " <font=nut_ChatFontAction>*"..value:sub(2, -2).."* </font>"
						end
					end)
				end
			end
	...
	local CHAT_X, CHAT_Y = NUT_CVAR_CHATX:GetInt() * 0.15, ScrH() * NUT_CVAR_CHATY:GetFloat() * 1.175;
	local CHAT_W, CHAT_H = ScrW() * 0.475, ScrH() * 0.4	
	...
	function nut.chat.AddText(...)
		local message = vgui.Create("nut_MarkupPanel", nut.chat.panel.content);
		message:Dock(TOP);
	...
	function nut.chat.CreatePanels()
		if (!IsValid(nut.chat.panel.frame)) then
			...
			frame:SetPos(CHAT_X, CHAT_Y * 1.79 - CHAT_H);
			frame:SetSize(CHAT_W, ScrH() - (CHAT_Y * 1.79 - CHAT_H) - 28);
			frame:SetDrawBackground(false);

			local content = frame:Add("AdvNut_ScrollPanel")
			content:Dock(FILL)
	...
	function nut.chat.Toggle(state)
		...
		if (state) then
			...
			entry:SetPos(CHAT_X, ScrH() - 26);
			entry:SetWide(CHAT_W)
			entry:MakePopup()
			entry:RequestFocus()
			entry:SetTall(24)
			...
			entry.OnTextChanged = function(panel)
				AdvNut.hook.Run("ChatTextChanged", panel:GetText())
			end

			AdvNut.hook.Run("ChatOpened")
	...
	netstream.Hook("nut_PlayerSay", function(client, data)
		if ((client.nextTalk or 0) < CurTime()) then
			...
			AdvNut.hook.Run("PlayerSay", client, data, true)
		end
	end)
end