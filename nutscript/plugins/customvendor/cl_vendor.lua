local PANEL = {}
	function PANEL:Init()
		...
		self:MakePopup();
		self:Center();
		self:SetTitle(nut.lang.Get("vd_vendor"));
	...
	function PANEL:SetEntity(entity)
		...
		if (vendorAction.sell) then
			if (self.selling == nil) then
				...
				self.sellingTab = self.property:AddSheet(nut.lang.Get("vd_sell"), self.selling, "icon16/coins_delete.png")
			end;
		...
		if vendorAction.buy then
			if (self.buying == nil) then
				...
				self.buyingTab = self.property:AddSheet(nut.lang.Get("vd_buy"), self.buying, "icon16/coins_add.png")
			end;
		...
		if (LocalPlayer():IsAdmin() and self.admin == nil) then
			...
			self.property:AddSheet(nut.lang.Get("vd_admin"), self.admin, "icon16/star.png");
		end
	end
	...
	function PANEL:SetEntity(entity, boolBuying)
		...
		for class, itemTable in SortedPairs(nut.item.GetAll()) do
			...
			if (data[class] and data[class].selling and !boolBuying ) then
				...
				icon:SetToolTip(nut.lang.Get("vd_sell_desc", itemTable.name, itemTable:GetDesc(), cost))
...
local PANEL = {}
	function PANEL:Init()
		...
		self.info:SetText(nut.lang.Get("vd_admin_tip"))
	...
	function PANEL:SetEntity(entity)
		...
		self.list:AddColumn(nut.lang.Get("vd_name"))
		self.list:AddColumn(nut.lang.Get("vd_itemID"))
		self.list:AddColumn(nut.lang.Get("vd_sell"))
		self.list:AddColumn(nut.lang.Get("vd_buy"))
		self.list:AddColumn(nut.lang.Get("vd_price"));
		self.list:AddColumn(nut.lang.Get("vd_orignal_price"));
		self.list:SetMultiSelect(false)
		self.list.OnClickLine = function(panel, line, selected)
			if (input.IsMouseDown(MOUSE_LEFT)) then
				local menu = DermaMenu()
				menu:AddOption( nut.lang.Get("vd_admin_buying"), function()
					line.buying = !line.buying
					line:SetValue(4, line.buying and "✔" or "")
				end):SetImage("icon16/money_add.png")
				menu:AddOption( nut.lang.Get("vd_admin_selling"), function()
		...
		self.list.OnRowRightClick = function(parent, index, line)
			Derma_StringRequest(line.itemTable.name, nut.lang.Get("vd_admin_ask_price"), "0", function(text)
		...
		faction:SetText(nut.lang.Get("vd_admin_faction"))
		...
		for k, v in SortedPairs(nut.faction.GetAll()) do
			...
			panel:Dock(TOP)
			panel:SetText(nut.lang.Get("vd_admin_faction_desc", v.name))
			panel:SetValue(0)
		...
		classes:SetText(nut.lang.Get("vd_admin_classes"))
		...
		for k, v in SortedPairs(nut.class.GetAll()) do
			local panel = self.scroll:Add("DCheckBoxLabel")
			panel:Dock(TOP)
			panel:SetText(nut.lang.Get("vd_admin_classes_desc", v.name))
			panel:SetValue(0)
			panel:DockMargin(12, 3, 3, 3)
			panel:SetDark(true)
		...
		name:SetText(nut.lang.Get("vd_admin_name"))
		...
		action:SetText(nut.lang.Get("vd_admin_action"))
		...
		self.sell:Dock(TOP)
		self.sell:SetText(nut.lang.Get("vd_admin_sell"))
		...
		self.buy:Dock(TOP)
		self.buy:SetText(nut.lang.Get("vd_admin_buy"))
		self.buy:SetValue(0)
		...
		adj:SetText(nut.lang.Get("vd_admin_adj"))
		...
		money:SetText(nut.lang.Get("vd_admin_money"))
		...
		desc:SetText(nut.lang.Get("vd_admin_desc"))
		...
		model:SetText(nut.lang.Get("vd_admin_model"))
		...
		self.save:SetText(nut.lang.Get("vd_admin_save"))
