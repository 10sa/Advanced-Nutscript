...
SWEP.Instructions = "왼쪽 클릭: 문 잠그기\n오른쪽 클릭: 문 잠금 해제"
SWEP.Purpose = "당신이 소유한 문에 사용되는 열쇠들입니다."
SWEP.Drop = false
...
function SWEP:PrimaryAttack()
	...
	if (IsValid(entity)) then
		...
		if (entity:GetNetVar("unownable") or entity:GetNetVar("owner") == nil) then return end;

		if (entity:GetNetVar("owner") != self.Owner:SteamID()) then
...
function SWEP:SecondaryAttack()
	...
	if (SERVER and IsValid(entity)) then
		if (!PLUGIN:IsDoor(entity)) then
			nut.util.Notify("이것은 올바른 문이 아닙니다.", self.Owner)
		...
		if (entity:GetNetVar("unownable") or entity:GetNetVar("owner") == nil) then return end;
		if (entity:GetNetVar("owner") != self.Owner:SteamID()) then
			nut.util.Notify("당신은 이 문의 주인이 아닙니다.", self.Owner)
